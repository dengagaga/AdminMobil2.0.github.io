...
function izi(item) {
  item.innerHTML += ` <tr class="tr-one-bot tr-one-bot-all tr-one-bot--active">
    ...
    <td class="td-one td-inps-gas"><div class="price-flex"><p class="baza-price">0</p> <span class="span-price span-price-bgc">₽</span></div></td>
    <td class="td-one"><div class="price-flex"><p class="baza-price-two">0 / 0</p><span class="span-price span-price-bgc">₽</span></div></td>
</tr>`;
...
// MOBIL
const burgerPriceClick = document.querySelector(
  ".burger-price-all-dropmenu-active-all"
);
const burgerPriceList = document.querySelector(
  ".burger-price-all-dropmenu-list"
);
const allPriceItemDrop = document.querySelectorAll(".all-price-item-drop");
const allPriceItemDropImg = document.querySelectorAll(
  ".all-price-item-drop-img"
);
const allPriceItemDropList = document.querySelectorAll(
  ".all-price-item-drop-list"
);
const allPriceItemDropAll = document.querySelectorAll(
  ".all-price-item-drop-all"
);
const allPriceItems = document.querySelector(".all-price-item-alls");
const allPriceItems2 = document.querySelector(".all-price-item-alls-2");
const btnAllAdd = document.querySelector(".all-price-item-btn");
const btnAllAdd2 = document.querySelector(".all-price-item-btn-2");
const allBTN = document.querySelector(".all-price-item-title-btn");
const allBTNOtm = document.querySelector(".all-price-item-mid-btn-otm");
...
const allpriceDropLast2 = document.querySelector(
  ".all-price-item-drop-all-last-2"
);
...
allPriceItemDrop.forEach((allDropItem) => {
  allDropItem.onclick = (e) => {
    const child1 = allDropItem.children[1];
    const child2 = e.currentTarget.parentElement.children[1];
    allBTN.classList.add("all-price-item-title-btn-active");
    child1.classList.toggle("allPriceItemDropImg--active");
    allDropItem.classList.toggle("all-price-item-drop--active");
    child2.classList.toggle("all-price-item-drop-list--none");
  };
});
allBTN.onclick = () => {
  if (allBTN.textContent == "Раскрыть") {
    ...
    allPriceItemDropList.forEach((lists) => {
      lists.classList.remove("all-price-item-drop-list--none");
    });
    ...
    allPriceItemDropList.forEach((lists) => {
      lists.classList.add("all-price-item-drop-list--none");
    });
    allPriceItemDropImg.forEach((img) => {
...
btnAllAdd.onclick = () => {
  allMid.classList.remove("all-price-item-mid--none");
  allPriceItems.insertAdjacentHTML(
    "beforeend",
    `<div class="all-price-item-drop-all all-price-item-drop-all-new ">
  <div
    class="all-price-item-drop all-price-item-drop--active-new all-new-1"
  >
    <div class="all-price-item-drop-left">
      <input
        type="checkbox"
        name=""
        id=""
        class="all-price-item-top-inp"
      />
      <p>Название комплекса</p>
    </div>
    <div class="all-price-item-drop-img">
      <svg
        width="6"
        height="7"
        viewBox="0 0 6 7"
  ...
  <ul
    class="all-price-item-drop-list all-price-item-drop-list-new"
  >
    <li class="item-description">
      <span class="item-description-span">Описание</span>
      <p class="item-description-text-click">Часть тела</p>
      <input type="text" class="input-display-none">

    </li>
    <li class="item-description">
      <span class="item-description-span"
        >Стоимость разовой процедуры</span
      >
      <div class="item-description-bot">
        <p class="item-description-text item-description-text-none">0</p>
        <input type="text" class="input-display-none-2">
        <span class="item-description-bot-span">₽</span>
      </div>
    </li>
    <li class="item-description">
      <span class="item-description-span"
        >Абонемент на 6 процедур</span
      >
      <div class="item-description-bot">
        <p class="item-description-text item-description-texts-none">0 / 0</p>
        <input type="text" class="input-display-none-3">
        <span class="item-description-bot-span">₽</span>
      </div>
    </li>
  </ul>
  ...
  const allItemsClick = document.querySelector(".item-description-text-click");
  const inputSisplayNone = document.querySelector(".input-display-none");
  const inputSisplayNone2 = document.querySelector(".input-display-none-2");
  const inputSisplayNone3 = document.querySelector(".input-display-none-3");
  const itemDescriptionTextNone = document.querySelector(
    ".item-description-text-none"
  );
  const itemDescriptionTextNones = document.querySelector(
    ".item-description-texts-none"
  );

  allItemsClick.onclick = () => {
    allItemsClick.style.display = "none";
    inputSisplayNone.style.display = "block";
  };
  itemDescriptionTextNone.onclick = () => {
    itemDescriptionTextNone.style.display = "none";
    inputSisplayNone2.style.display = "block";
  };
  itemDescriptionTextNones.onclick = () => {
    itemDescriptionTextNones.style.display = "none";
    inputSisplayNone3.style.display = "block";
  };
  const allItemDrop = document.querySelector(".all-new-1");
  const allItemDropPareent = allItemDrop.parentElement;
  allItemDrop.onclick = () => {
    allItemDrop.classList.remove("all-price-item-drop--active-new");
    allItemDropPareent.classList.remove("all-price-item-drop-all-new");
    allItemDropPareent.children[1].classList.toggle(
      "all-price-item-drop-list--none"
    );
  };
};
btnAllAdd2.onclick = () => {
  allMid2.classList.remove("all-price-item-mid--none");

  allPriceItems2.insertAdjacentHTML(
    "beforeend",
    `<div class="all-price-item-drop-all all-price-item-drop-all-new ">
    <div
      class="all-price-item-drop all-price-item-drop--active-new all-new-2"
    >
      <div class="all-price-item-drop-left">
        <input
          type="checkbox"
  ...
  const allItemDrop = document.querySelector(".all-new-2");
  const allItemDropPareent = allItemDrop.parentElement;
  allItemDrop.onclick = () => {
    allItemDrop.classList.remove("all-price-item-drop--active-new");
    allItemDropPareent.classList.remove("all-price-item-drop-all-new");
...
allBTNOtm.onclick = () => {
  allMid.classList.add("all-price-item-mid--none");
};
allBTNOtm2.onclick = () => {
  allMid2.classList.add("all-price-item-mid--none");
};
